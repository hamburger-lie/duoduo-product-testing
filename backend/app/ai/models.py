...
class ModelRouter:
    ...
    def __init__(self) -> None:
        ...
        if provider == "deepseek":
            ...
            # 产品理解（base64 兜底描述）：豆包优先，其次智谱 GLM-4.6V
            vision_provider = getattr(s, "vision_provider", "").strip().lower()
            zhipu_key = getattr(s, "zhipu_api_key", "")
            if vision_provider == "doubao" and getattr(s, "ark_api_key", ""):
                self._routes[TaskType.PRODUCT_UNDERSTAND] = ModelRoute(
                    task_type=TaskType.PRODUCT_UNDERSTAND,
                    endpoint_id=getattr(s, "ark_model_vision", "doubao-seed-2-0-mini-260428"),
                    endpoint_env_name="ARK_MODEL_VISION",
