...
@router.post("/daily-login", response_model=DailyLoginCreditResponse)
async def claim_daily_login_reward(
    current_user: User = current_user_dependency,
    session: AsyncSession = db_session_dependency,
) -> DailyLoginCreditResponse:
    """Grant 10 credits for today's login, once per user per day."""

    awarded, amount, balance = await CreditService(session).claim_daily_login_reward(current_user)
    return DailyLoginCreditResponse(
        awarded=awarded,
        amount=amount,
        balance=balance,
    )
