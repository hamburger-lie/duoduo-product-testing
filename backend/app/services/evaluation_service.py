...
RUNNING_STATUSES = {"queued", "answering", "generating_report"}
...
def can_cancel_evaluation(status_value: str) -> bool:
    ...
    return status_value in {"pending", "generating_survey", "queued", "answering"}
...
class EvaluationService:
    ...
    async def run_evaluation(
        ...
        *,
        ...
        running_count = await EvaluationRepository(self.session).count_running_by_user_id(
            user_id=user.id,
        )
        max_running = settings.max_running_evaluations_per_user
        if running_count >= max_running:
            raise AppException(
                code="TOO_MANY_RUNNING_EVALUATIONS",
                message=f"您已有 {running_count} 个评测正在运行中，请等待完成后再提交新的评测",
                http_status=status.HTTP_429_TOO_MANY_REQUESTS,
                details={
                    "running_count": running_count,
                    "max_allowed": max_running,
                },
            )
    ...
    async def _run_evaluation_sync(
        ...
        *,
        ...
        """Mark evaluation as answering (Phase 1). AI work is done in background."""

        evaluation = await self._get_owned_evaluation(user=user, evaluation_id=evaluation_id)
        if evaluation.survey_id is None:
            raise self._evaluation_not_ready(evaluation_id)
        ...
        return EvaluationRunResponse(
            ...
            estimated_seconds=0,
            task_id=f"bg_task_{evaluation.id}",
        )

    async def _run_evaluation_ai_work(
        self,
        *,
        user: User,
        evaluation_id: int,
        ...
        """Phase 2: generate AI answers and mark done. Runs in its own session."""
        ...
        # 依次写库
        for persona, result in zip(personas_to_run, ai_results, strict=False):
        ...
        # is LLM-scored by the time the user opens the report. Uses its own DB
        # session internally, so it is safe after this request's session closes.
        from app.services.report_service import ReportService

        asyncio.create_task(
            ReportService(self.session).generate_dimension_analysis(
                evaluation_id=evaluation.id
            )
        )
    ...
    async def delete_evaluation(self, *, user: User, evaluation_id: int) -> None:
        """Soft-delete an owned evaluation so history no longer shows it."""

        evaluation = await self._get_owned_evaluation(user=user, evaluation_id=evaluation_id)
        await self.evaluations.soft_delete(evaluation)
        await self.session.commit()
