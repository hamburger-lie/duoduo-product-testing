...
from app.storage.adapters import LocalProductStorageAdapter, MockProductStorageAdapter, ProductStorageAdapter
...
class ProductService:
    ...
    def __init__(
        ...
        self._storage_adapter = storage_adapter or _make_storage_adapter()
    ...
    async def create_product(self, *, user: User, payload: ProductCreateRequest) -> ProductResponse:
        ...
        # Backend-side link merge: if the client didn't forward extract context
        # but uploaded images, reconstruct it from the extract cache (or one
        # guarded extract) so product understanding — and the personas
        # downstream — get appearance/raw_text. No frontend change required.
        if payload.image_extract is None and payload.image_object_keys:
            payload.image_extract = await self._resolve_image_extract(
                payload.image_object_keys
            )
    ...
    async def _resolve_image_extract(self, object_keys: list[str]):
        """Reconstruct vision-extract context from uploaded image object keys.

        Reuses ProductImageExtractService (content-hash cache + guarded vision
        client): a cache hit (the usual upload→create flow) costs no vision
        call; a miss runs exactly one guarded extract. Failures degrade to
        text-only understanding rather than breaking product creation.
        """

        from app.schemas.product import ImageExtractContext, ImageExtractRequest
        ...
        try:
            urls = [
                self._storage_adapter.build_public_url(key) for key in object_keys
            ]
            resp = await ProductImageExtractService().extract(
                ImageExtractRequest(image_urls=urls)
            )
        except Exception:
            logger.warning("resolve_image_extract_failed", exc_info=True)
            return None

        appearance_field = resp.fields.get("appearance")
        appearance = appearance_field.value if appearance_field else None
        ctx = ImageExtractContext(
            raw_text=resp.raw_text,
            appearance=appearance if isinstance(appearance, str) else None,
            suggested_description=resp.suggested_description,
        )
        if not (ctx.raw_text or ctx.appearance or ctx.suggested_description):
            return None
        return ctx
