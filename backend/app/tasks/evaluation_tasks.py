            ...
            if evaluation.status == "done":
                try:
                    generate_report_task.apply_async(
                        args=[evaluation_id, user_id],
                        countdown=2,  # small delay so commit is visible
                    )
                    logger.info(
                        "report_pregeneration_enqueued",
                        extra={
                            "event": "report_pregeneration_enqueued",
                            "evaluation_id": evaluation_id,
                            "user_id": user_id,
                        },
                    )
                except Exception as exc_report:
                    # Non-fatal: user can still trigger report on demand
                    logger.warning(
                        "report_pregeneration_enqueue_failed",
                        extra={
                            "event": "report_pregeneration_enqueue_failed",
                            "evaluation_id": evaluation_id,
                            "error_message": str(exc_report)[:200],
                        },
                    )
...
def generate_report_task(self: object, evaluation_id: int, user_id: int) -> dict[str, object]:
    """Celery task that pre-generates a report after evaluation completes.

    This runs in the worker so API processes are not blocked by heavy
    report computation. If a report already exists, this is a no-op.
    """
    ...
    logger.info(
        "report_task_started",
        extra={
            "event": "report_task_started",
            "evaluation_id": evaluation_id,
            "user_id": user_id,
            "task_id": str(self.request.id),
        },
    )
    result = _run_async(_generate_report_async(evaluation_id, user_id))

    from app.core.metrics import record_celery_task

    task_status = str(result.get("status", "unknown"))
    record_celery_task("report.generate", task_status)
    return result
...
async def _generate_report_async(
    ...
    """Async implementation of report pre-generation."""
    ...
    async with AsyncSessionFactory() as session:
        # Check if report already exists — if so, skip
        reports = ReportRepository(session)
        existing = await reports.get_by_evaluation_id(
            evaluation_id=evaluation_id,
            include_deleted=False,
        )
        if existing is not None:
            logger.info(
                "report_task_skipped_exists",
                extra={
                    "event": "report_task_skipped_exists",
                    "evaluation_id": evaluation_id,
                },
            )
        ...
        if user is None:
            logger.warning(
                "report_task_user_not_found",
                extra={
                    "event": "report_task_user_not_found",
                    "evaluation_id": evaluation_id,
                    "user_id": user_id,
                },
            )
        ...
        try:
            ...
            await svc.get_or_create_report(
                user=user,
                evaluation_id=evaluation_id,
            )
            logger.info(
