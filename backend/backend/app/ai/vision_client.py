...
def _is_private_host(host: str) -> bool:
    ...
    try:
        ...
        return False  # Not an IP literal — hostname; allow DNS to resolve
...
class ZhipuVisionClient:
    ...
    async def extract_fields(
        ...
        *,
        ...
        fields_list = ", ".join(f'"{f}"' for f in effective_fields)
        user_prompt = (
            "请仔细分析图片中的产品，提取所有可见的产品信息。\n\n"
            "输出要求：\n"
            "- 只输出 JSON，不要输出任何其他文字\n"
            "- raw_text：把图片上你能看到的所有真实文字原样抄写下来\n"
            "- suggested_description：根据识别到的信息写 100-200 字产品描述\n"
            f"- fields 中包含这些字段：{fields_list}\n"
            "- 每个字段格式：{\"value\": ..., \"confidence\": 0.0-1.0, \"source\": \"zhipu_vision\"}\n"
            "- 识别不到的字段 value 填 null，confidence 填 0.0\n"
            "- 禁止编造图片中没有的内容"
        )
        ...
        result = self._parse_response(raw_response, len(image_urls), effective_fields)
        logger.info(
            "[perf] zhipu extract_fields total=%.1fms parse=%.1fms images=%d",
        ...
        *,
        ...
        async with _httpx.AsyncClient(
            ...
            for idx, img_url in enumerate(image_urls):
                ...
                img_bytes = _compress_image_for_vision(img_bytes, ct)
    ...
    def _parse_response(
        ...
        for field_name in target_fields:
            ...
            if fd and isinstance(fd, dict):
                ...
                source = str(fd.get("source", "zhipu_vision"))
...
def get_vision_extract_client() -> VisionClientProtocol:
    ...
    if provider == "zhipu":
        ...
        if mode == "vision_text":
            logger.info("image_extract_mode=vision_text (two-step: zhipu OCR + deepseek structuring)")
            return ZhipuVisionTextClient(zhipu)

        # Default: full vision extraction
        return zhipu

    raise AIServiceUnavailable(
        f"Unknown VISION_PROVIDER {provider!r}. Valid values: 'mock', 'zhipu'."
    )
