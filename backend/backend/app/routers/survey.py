...
def survey_generation_progress(elapsed_seconds: float) -> tuple[int, str]:
    """Return a moving progress hint while DeepSeek is still generating."""

    seconds = max(0, int(elapsed_seconds))
    if seconds < 8:
        return 24 + seconds * 3, f"DeepSeek 正在设计问卷结构，已等待 {seconds} 秒"
    if seconds < 20:
        return 48 + (seconds - 8) * 2, f"DeepSeek 正在生成题目内容，已等待 {seconds} 秒"
    if seconds < 45:
        return 72 + int((seconds - 20) * 0.6), f"DeepSeek 正在校验题型和维度，已等待 {seconds} 秒"
    if seconds < 90:
        return 88 + int((seconds - 45) * 0.2), f"DeepSeek 正在整理完整问卷，已等待 {seconds} 秒"
    return 98, f"DeepSeek 仍在生成，请继续等待，已等待 {seconds} 秒"
