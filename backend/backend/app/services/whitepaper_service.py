...
class WhitepaperService:
    ...
    async def _call_proxy(self, text: str) -> str:
        ...
        system = (
            "你是一位资深美业市场研究分析师，擅长将消费者调研数据转化为专业的行业白皮书。\n\n"
            "请根据提供的产品调研数据，生成一份结构完整、逻辑严密的美业产品数据白皮书（Markdown 格式）。\n\n"
            "白皮书要求：\n"
            "1. 使用标准 Markdown 格式（# ## ### 标题层级，**加粗**，数字/无序列表）\n"
            "2. 章节结构：执行摘要 → 市场背景 → 产品概况 → 消费者洞察 → 数据分析 → "
            "卖点与顾虑 → 目标受众 → 市场建议 → 结论\n"
            "3. 语言专业、客观，使用行业术语，避免口语化\n"
            "4. 严格基于所提供的数据进行分析，不虚构数据，可适当补充行业背景\n"
            "5. 全文约 2000-3000 字\n"
            "6. 直接输出 Markdown 正文，不要在开头或结尾添加任何解释性文字"
        )
