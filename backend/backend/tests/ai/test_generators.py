...
def _make_valid_survey_json(n: int = 30) -> str:
    questions = [
        {
            "id": f"q{i:02d}",
...
class _GoodSurveyClient(MockAIClient):
    """MockAIClient that returns a valid 30-question survey JSON."""
...
async def test_persona_answer_ark_path_returns_valid_tuple(
    ...
    (answers, intent, sentiment, summary_comment, thinking_process) = (
...
async def test_persona_answer_ark_path_clamps_intent_to_1_5(
    ...
    _, intent, _, _sc, _ = await svc._generate_answer_with_ai(
        survey=_fake_survey(),
        persona=_fake_persona(),
        product_summary={},
    )
