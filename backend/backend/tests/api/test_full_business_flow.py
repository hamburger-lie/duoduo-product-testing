...
async def test_complete_happy_path(api_client: AsyncClient) -> None:
    ...
    # 7. Run evaluation (202 = accepted; sync mode runs AI in background task,
    # which httpx completes before returning, so status will be "done" by the
    # time we read credits/report/answers below)
    resp = await api_client.post(
        f"/api/v1/evaluations/{eval_id}/run",
        headers=headers,
    )
    assert resp.status_code == 202
    assert resp.json()["status"] in {"answering", "done"}
