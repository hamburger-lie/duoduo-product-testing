...
async def run_single_test(
    ...
    try:
        ...
        print(f"    [ERROR] {persona['name']} × {product['name']}: {e}")
        return None
...
async def main():
    ...
    # ── Phase 1: Score Matrix (5 personas × 5 products) ──
    print("=" * 60)
    print("PHASE 1: Score Matrix (5 personas × 5 products = 25 calls)")
    print("=" * 60)

    scores = {}  # persona_name -> {product_name -> score}
    sentiments = {}
    verdicts = {}
    ...
    for persona in personas:
        ...
        for product in PRODUCTS:
            ...
            print(f"  {pname} × {prodname} (RMB{product['price']})...", end=" ", flush=True)
    ...
    print("SCORE MATRIX")
    print("=" * 60)
    header = f"{'角色':<8}" + "".join(f"{'|' + p['name'][:8]:<12}" for p in PRODUCTS)
    print(header)
    print("-" * len(header))
    for persona in personas:
        pname = persona["name"]
        row = f"{pname:<8}"
        for product in PRODUCTS:
            s = scores[pname].get(product["name"], "?")
            row += f"|{str(s):^11}"
        print(row)
    ...
    if all_scores:
        ...
        print(f"平均分: {avg:.2f}")
        print(f"分数分布: {dist}")
        print(f"购买率(≥4分): {purchase}/{len(all_scores)} = {purchase/len(all_scores)*100:.1f}%")
        ...
        for persona in personas:
            ...
            if pscores:
                ...
                print(f"  {pname}: 平均{pavg:.1f}, 购买率{ppurchase}/{len(pscores)}")

    # ── Phase 2: Context Card Test (2 personas × 1 product × 3 contexts) ──
    print("\n" + "=" * 60)
    print("PHASE 2: Context Card Test (场景卡片影响)")
    print("=" * 60)

    test_personas = [personas[1], personas[3]]  # 陈婷婷(价格敏感) + 王佳怡(学生党)
    test_product = PRODUCTS[0]  # 珀莱雅 RMB99

    for persona in test_personas:
        pname = persona["name"]
        print(f"\n  {pname} × {test_product['name']}:")
        for ctx_name, ctx_text in CONTEXT_CARDS.items():
            print(f"    场景[{ctx_name}]...", end=" ", flush=True)
