...
def load_personas() -> list[dict]:
    """Load all beauty persona JSON files."""
    personas = []
...
PRODUCTS = [
    {
        ...
        "price": 99,
        "description": (
            "含虾青素、麦角硫因，主打抗氧抗糖双抗功效，轻薄水润质地，"
            "适合日常护肤第一步。天猫销量榜前五，小红书10万+笔记。"
        ),
    ...
    },
    {
        ...
        "price": 680,
        "description": (
            "15%左旋VC+1%VE+0.5%阿魏酸，Duke大学专利配方，"
            "有多项临床试验数据支持抗氧化和光老化修护。"
            "质地偏油，需要建立耐受。"
        ),
    ...
    },
    {
        ...
        "price": 149,
        "description": (
            "0.1%视黄醇+神经酰胺，主打夜间抗皱修护。新锐国货，"
            "小红书口碑好但部分用户反映有轻微刺激。京东销量榜前十。"
        ),
...
CONTEXT_CARDS = {
    ...
    "month_end": (
        "这个月花超了，信用卡账单还没还完。"
        "本来说这个月不买护肤品了，"
        "但刚好刷到直播间在推这个产品，主播说库存只剩最后一批了。"
    ),
    "friend_rec": (
        "闺蜜/室友刚买了这个产品，在微信群里发了使用照片说特别好用，"
        "还说'你肯定会喜欢'，@了你好几次让你也买。"
    ),
}
...
async def main() -> None:  # noqa: C901, PLR0912, PLR0915
    """Run the full scoring stress test."""
    ...
    for persona in personas:
        ...
        for product in PRODUCTS:
            ...
            print(
                f"  {pname} x {prodname} (RMB{product['price']})...",
                end=" ",
                flush=True,
            )
            result = await run_single_test(
                client, endpoint_id, persona, product,
            )
            if result:
    ...
    print("=" * 60)
    prod_short = [
        "polaiya99", "dabao29", "xiulk680", "winona198", "hbn149",
    ]
    ...
    all_scores = [
        v
        for pscores in scores.values()
        for v in pscores.values()
        if isinstance(v, int)
    ]
    if all_scores:
        avg = sum(all_scores) / len(all_scores)
        dist = {n: all_scores.count(n) for n in range(1, 6)}
        purchase = sum(1 for s in all_scores if s >= 4)
        pct = purchase / len(all_scores) * 100
        print(f"Average: {avg:.2f}")
        print(f"Distribution: {dist}")
        print(f"Purchase rate (>=4): {purchase}/{len(all_scores)} = {pct:.1f}%")
        ...
        for persona in personas:
            pname = persona["name"]
            pscores = [
                v for v in scores[pname].values() if isinstance(v, int)
            ]
            if pscores:
                ...
                print(
                    f"  {pname}: avg={pavg:.1f}, "
                    f"purchase={ppurchase}/{len(pscores)}"
                )
    ...
    male_product = {
        ...
        "price": 79,
        "description": (
            "男士专研控油配方，含火山矿物+活性炭，深层清洁控油，"
            "洗后清爽不紧绷。京东男士洁面销量榜Top3，4.9分好评。"
            "一步搞定清洁控油。"
        ),
    ...
    liu_xiao = personas[4]
    print(
        f"  {liu_xiao['name']} x {male_product['display']}"
        f" (RMB{male_product['price']})...",
        end=" ",
        flush=True,
    )
    result = await run_single_test(
        client, endpoint_id, liu_xiao, male_product,
    )
    if result:
        score = result.get("overall_intent", "?")
        sent = result.get("sentiment", "?")
    ...
    test_personas = [personas[1], personas[3]]
