...
def test_render_survey_generate_requires_product_specific_anchors() -> None:
    rendered, _, _ = render_prompt(
        "survey_generate",
        user_role_type="manufacturer",
        product_ai_summary={
            ...
            "brand": "欧莱雅",
            "price": 99,
            "main_selling_points": ["温和清洁", "洗后不紧绷"],
            "key_ingredients": ["氨基酸表活", "玻尿酸"],
            "competitive_position": "同价位洁面竞品",
            "usage_scenarios": ["晨间洁面", "晚间卸防晒后清洁"],
        },
        product={"id": 1},
    )

    assert "产品锚点配额" in rendered
    assert '禁止只把"这款产品"替换成产品名' in rendered
    assert "品牌/背书" in rendered
    assert "成分/配方" in rendered
    assert "真实竞品或替代选择" in rendered
    assert "使用场景" in rendered
