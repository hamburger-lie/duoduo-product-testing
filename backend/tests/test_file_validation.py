...
def test_validate_image_bytes_accepts_real_image_header() -> None:
    assert validate_image_bytes(b"\xff\xd8\xff\xe0" + b"\x00" * 32) == "image/jpeg"


def test_validate_image_bytes_rejects_executable_header() -> None:
    assert validate_image_bytes(b"MZ\x90\x00" + b"\x00" * 32) is None


def test_validate_pdf_bytes_accepts_pdf_header() -> None:
    assert validate_pdf_bytes(b"%PDF-1.7\n1 0 obj\n<<>>\nendobj\n") is True


def test_validate_pdf_bytes_rejects_html_payload() -> None:
    assert validate_pdf_bytes(b"<html><script>alert(1)</script></html>") is False
