...
async def test_create_product_rejects_oversized_base64_image(
    product_client: AsyncClient,
) -> None:
    token = await login(product_client, "mock_product_base64_large")
    response = await product_client.post(
        "/api/v1/products",
        headers={"Authorization": f"Bearer {token}"},
        json={
            "description": "This product description is long enough for validation.",
            "image_object_keys": [],
            "image_base64_list": ["A" * (11 * 1024 * 1024 + 1)],
        },
    )
