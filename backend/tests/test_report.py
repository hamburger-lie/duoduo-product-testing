...
async def test_upload_report_pdf_rejects_non_pdf_bytes(
    report_context: ReportContext,
) -> None:
    token = await login(report_context, "report_pdf_invalid")
    product_id = await create_product(report_context, token, "PDF invalid product")
    evaluation = await create_evaluation(report_context, token, product_id)
    evaluation_id = str(evaluation["id"])

    response = await report_context.client.post(
        f"/api/v1/reports/by-evaluation/{evaluation_id}/pdf",
        headers={"Authorization": f"Bearer {token}"},
        files={"file": ("report.pdf", b"<html>not a pdf</html>", "application/pdf")},
    )
...
async def test_upload_report_pdf_rejects_oversized_pdf(
    report_context: ReportContext,
) -> None:
    token = await login(report_context, "report_pdf_large")
    product_id = await create_product(report_context, token, "PDF large product")
    evaluation = await create_evaluation(report_context, token, product_id)
    evaluation_id = str(evaluation["id"])
...
async def test_upload_report_pdf_base64_rejects_invalid_base64(
    report_context: ReportContext,
) -> None:
    token = await login(report_context, "report_pdf_bad_base64")
    product_id = await create_product(report_context, token, "PDF base64 product")
    evaluation = await create_evaluation(report_context, token, product_id)
    evaluation_id = str(evaluation["id"])

    response = await report_context.client.post(
        f"/api/v1/reports/by-evaluation/{evaluation_id}/pdf-base64",
        headers={"Authorization": f"Bearer {token}"},
        json={"pdf_base64": "not-valid-base64!!!", "filename": "report.pdf"},
    )
