...
Page({
  ...
  _extractProgressTimer: null as ReturnType<typeof setInterval> | null,
  ...
  _setExtractProgress(pct: number, msg: string) {
    this.setData({ extractProgress: Math.round(pct), extractMsg: msg });
  },

  _animateExtractTo(targetCap: number, intervalMs = 500) {
    if (this._extractProgressTimer) clearInterval(this._extractProgressTimer);
    this._extractProgressTimer = setInterval(() => {
      const cur = this.data.extractProgress;
      if (cur >= targetCap) return;
      this.setData({ extractProgress: Math.min(targetCap, cur + 1) });
    }, intervalMs);
  },

  _stopExtractProgress() {
    if (this._extractProgressTimer) {
      clearInterval(this._extractProgressTimer);
      this._extractProgressTimer = null;
    }
  },
  ...
  async onExtractFromImages() {
    ...
    try {
      ...
      this._setExtractProgress(12, '上传图片中');
      this._animateExtractTo(35, 500); // upload ~8-10s, 23 steps × 500ms ≈ 11.5s
      ...
      this._setExtractProgress(42, 'AI 正在分析图片');
      this._animateExtractTo(82, 800); // AI ~30-35s, 40 steps × 800ms ≈ 32s
      ...
      this._setExtractProgress(88, '正在填充产品信息');
      this._animateExtractTo(96);
      await this._applyExtractResult(result);
      this._stopExtractProgress();
      this._setExtractProgress(100, '识别完成');
  ...
  async _applyExtractResult(result: ImageExtractResponse) {
    ...
    // If nothing was filled into form fields, treat as a soft failure
    const anyFilled = Object.keys(pendingUpdates).length > 0;
    if (!anyFilled) {
      this.setData({
        extractDone: false,
        extractRawText: result.raw_text || '',
        rawTextExpanded: !!result.raw_text,
        extractError: '未能提取产品信息，请参考原始文字手动填写',
      });
      return;
    }
