...
import { avatarSrcForSlot, avatarSlotForPersona, fixedSlotForPersona } from '../../utils/personaAvatar';
...
function readCachedReport(evalId: string): CachedReportSnapshot | null {
  try {
    ...
    if (!hasCompleteRawRadarDims(parsed.report)) {
      console.log('[radar-real] discard stale report cache:', {
        evaluationId: evalId,
...
function writeCachedReport(evalId: string, snapshot: CachedReportSnapshot): void {
  try {
    if (!hasCompleteRawRadarDims(snapshot.report)) {
      console.log('[radar-real] skip sparse report cache:', {
        evaluationId: evalId,
...
// 调试开关：开发时设为 true 查看完整日志，正式发布前改为 false
const DEBUG_RADAR = true;
...
function buildConclusion(
  ...
  if (compositeGrade === 'B') {
    if (top2BoxPct === 0) return { text: `购买意愿中性，${risk}是主要拦截项。` };
    return { text: `购买兴趣成立，但${risk}仍需补强。` };
  }
...
function splitPersonaQuote(
  ...
  const riskText = personaHesitationText(score, risks);
  if (!cleaned) {
    return {
  ...
  return {
    ...
    hesitation: compactVoiceText(riskText || findHesitationSentence(cleaned) || personaHesitationText(score, risks)),
  };
}

function findHesitationSentence(text: string): string {
  const parts = cleanReportText(text || '')
    .split(/[。；;，,]/)
    .map(p => p.trim())
    .filter(Boolean);
  const matched = parts.find(part => /担心|顾虑|犹豫|怕|但是|但|不过|不太|不够|不足|太贵|价格|刺激|过敏|效果|品牌|信任|成分|安全|复购/.test(part));
  return matched || '';
}
...
function buildPersonaVoices(
  ...
  return [...answers]
    ...
    .map((item, index) => {
      ...
      const avatarSlot = avatarSlotForPersona(item.persona_id || '', index, usedSlots, {
        persona_tag: item.persona_tag,
        name: item.persona_name,
        avatar: String((item as any).avatar ?? ''),
      });
      return {
        ...
        avatar: avatarSrcForSlot(avatarSlot),
      };
    });
}
...
function extractShortTitle(title: string): string {
  ...
  // Handle "维度名：内容片段" backend format — return label before colon
  m = t.match(/^([^：]{1,6})：/);
  if (m) return m[1];
...
function normalizeDeepAnalysis(raw: DeepAnalysis): DeepAnalysisView {
  return {
    ...
    sections: (raw.sections || []).map(section => {
      const contentParagraphs = splitDeepParagraphs(section.content);
      const highlight = buildDeepHighlightText(section);
      return {
        ...section,
...
Page({
  ...
  buildBaseVM(report: BackendReport, productName: string, tagMap: Record<string, string> = {}, answers: Array<{ persona_tag?: string; overall_intent?: number }> = []) {
    ...
    return {
      ...
      personaVoices: (() => {
        ...
        const fbUsed = new Set<number>();
        return [...segs].sort((a, b) => b.avg_intent - a.avg_intent).slice(0, 5).map((seg, i) => {
          const segLabel = pickSegmentLabel(seg.segment, i, tagMap, sortedA);
          const fbSlot = avatarSlotForPersona('', i, fbUsed, { persona_tag: seg.segment });
          return {
          groupTitle: compactGroupTitle(segLabel, `群体${ORDINAL_ZH[i] ?? i + 1}`),
          ...
          tone: tones[i] || 'slate',
          avatar: avatarSrcForSlot(fbSlot),
          };
        });
      })(),
      nextSteps: (() => {
  ...
  buildVM(report: BusinessReport, productName: string, tagMap: Record<string, string> = {}, answers: Array<{ persona_tag?: string; overall_intent?: number }> = []) {
    ...
    return {
      ...
      heroAdviceText: buildHeroAdvice(buildCompositeScore(scoredDims, avg).compositeGrade),
      topOpportunityText: joinTopSignals(dedupedOpportunities.slice(0, 1), '高意向卖点'),
      topRiskText: joinTopSignals(dedupedRisks.slice(0, 1), '决策证据不足'),
      personaVoices: (() => {
        ...
        const tones = ['violet', 'green', 'amber', 'blue', 'slate'];
        const sortedA = [...(answers as any[])].sort((a: any, b: any) => (b.overall_intent ?? 0) - (a.overall_intent ?? 0));
        const fbUsed2 = new Set<number>();
        return [...segs].sort((a, b) => b.avg_intent - a.avg_intent).slice(0, 5).map((seg, i) => {
          const segLabel2 = pickSegmentLabel(seg.segment, i, tagMap, sortedA);
          const fbSlot2 = avatarSlotForPersona('', i, fbUsed2, { persona_tag: seg.segment });
          return {
          groupTitle: compactGroupTitle(segLabel2, `群体${ORDINAL_ZH[i] ?? i + 1}`),
          score: `${seg.avg_intent.toFixed(1)}/5`,
          attractedBy: joinTopSignals(dedupedOpportunities.slice(0, 1), '核心卖点'),
          hesitation: joinTopSignals(dedupedRisks.slice(0, 1), '决策证据'),
          tone: tones[i] || 'slate',
          avatar: avatarSrcForSlot(fbSlot2),
          };
        });
      })(),
      nextSteps,
