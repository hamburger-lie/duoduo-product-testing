...
import type {
  CursorPaged, User, CreditBalance, CreditTransaction, DailyLoginCreditReward,
...
export const api = {
  ...
  async logout(): Promise<void> {
    ...
    await request<void>({ url: E.AUTH_LOGOUT, method: 'POST' });
  },
  ...
  async claimDailyLoginCredits(): Promise<DailyLoginCreditReward> {
    if (USE_MOCK) return delay({ awarded: true, amount: 10, balance: MOCK_BALANCE.balance + 10 });
    return request<DailyLoginCreditReward>({
      url: E.CREDIT_DAILY_LOGIN,
      method: 'POST',
    });
  },
